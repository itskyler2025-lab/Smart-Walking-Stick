...
<head>
    ...
    <style>
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #222831; color: #EEEEEE; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; }
        ...
        input { width: 100%; padding: 12px; margin-bottom: 15px; border: 1px solid #00ADB5; border-radius: 5px; background-color: #222831; color: #EEEEEE; box-sizing: border-box; }
...
void startConfigMode() {
  ...
  server.onNotFound([]() {
    server.send(302, "text/plain", "http://192.168.4.1"); // Redirect to config page
  });
  server.begin();
...
bool connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("\nWiFi connection failed. Entering configuration mode.");
    startConfigMode();
...
void loop() {
    ...
    switch (buzzerState) {
        ...
        case BUZZER_BEEP_2:
            ...
            break;
        case BUZZER_IDLE:
            // Do nothing, wait for an obstacle to trigger the sequence.
            break;
    }
